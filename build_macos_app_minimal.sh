...
# 清理构建目录
echo "🧹 清理构建目录..."
rm -rf build_macos_app_minimal
rm -rf dist/ChatMonitor_minimal
rm -rf dist/ChatMonitor_minimal.app

# 创建构建目录
echo "📁 创建构建目录: build_macos_app_minimal"
mkdir -p build_macos_app_minimal
...
# 构建 macOS 应用程序（最小优化版本）
echo "🔨 构建macOS应用程序（最小优化版本）..."

# 最小优化的 PyInstaller 命令，只排除最明显不需要的依赖
pyinstaller_cmd="python3 -m PyInstaller \
    --onedir \
    --windowed \
    --name=ChatMonitor_minimal \
    --noconfirm \
...
if [ $? -eq 0 ]; then
    echo "✅ 可执行文件创建成功: dist/ChatMonitor_minimal/ChatMonitor_minimal"
else
    echo "❌ 构建失败"
    exit 1
fi
...
# 创建.app包
echo "📦 创建.app包..."
app_dir="dist/ChatMonitor_minimal.app"
mkdir -p "$app_dir/Contents/MacOS"
mkdir -p "$app_dir/Contents/Resources"

# 复制可执行文件
cp "dist/ChatMonitor_minimal/ChatMonitor_minimal" "$app_dir/Contents/MacOS/ChatMonitor_minimal"
...
<dict>
    ...
    <string>ChatMonitor_minimal</string>
...
# 创建 DMG 安装包
echo "📦 创建DMG安装包..."
dmg_name="ChatMonitor-macOS-minimal-v1.0.0.dmg"
...
echo "  2. 将 ChatMonitor_minimal.app 拖拽到应用程序文件夹"
echo "  3. 从启动台或应用程序文件夹启动"
echo ""
echo "⚠️  注意: 首次运行可能需要在系统偏好设置中允许运行"
echo ""
echo "📊 最小优化策略:"
echo "  - 保留: 所有 YOLO 相关依赖 (torch, ultralytics, scipy)"
echo "  - 使用 --collect-all 确保完整收集 ultralytics 和 torch"
echo "  - 排除: 明显不需要的依赖 (paddle, matplotlib, pandas 等)"
echo "  - 预期大小: 约 700-900MB" 