...
# 清理构建目录
echo "🧹 清理构建目录..."
rm -rf build_macos_app_conservative
rm -rf dist/ChatMonitor_conservative
rm -rf dist/ChatMonitor_conservative.app

# 创建构建目录
echo "📁 创建构建目录: build_macos_app_conservative"
mkdir -p build_macos_app_conservative
...
# 构建 macOS 应用程序（保守优化版本）
echo "🔨 构建macOS应用程序（保守优化版本）..."

# 保守优化的 PyInstaller 命令，只排除明显不需要的依赖
pyinstaller_cmd="python3 -m PyInstaller \
    --onedir \
    --windowed \
    --name=ChatMonitor_conservative \
    --noconfirm \
...
if [ $? -eq 0 ]; then
    echo "✅ 可执行文件创建成功: dist/ChatMonitor_conservative/ChatMonitor_conservative"
else
    echo "❌ 构建失败"
    exit 1
fi
...
# 创建.app包
echo "📦 创建.app包..."
app_dir="dist/ChatMonitor_conservative.app"
mkdir -p "$app_dir/Contents/MacOS"
mkdir -p "$app_dir/Contents/Resources"

# 复制可执行文件
cp "dist/ChatMonitor_conservative/ChatMonitor_conservative" "$app_dir/Contents/MacOS/ChatMonitor_conservative"
...
<dict>
    ...
    <string>ChatMonitor_conservative</string>
...
# 创建 DMG 安装包
echo "📦 创建DMG安装包..."
dmg_name="ChatMonitor-macOS-conservative-v1.0.0.dmg"
...
echo "  2. 将 ChatMonitor_conservative.app 拖拽到应用程序文件夹"
echo "  3. 从启动台或应用程序文件夹启动"
echo ""
echo "⚠️  注意: 首次运行可能需要在系统偏好设置中允许运行"
echo ""
echo "📊 保守优化策略:"
echo "  - 保留: torch, ultralytics, scipy (YOLO必需)"
echo "  - 排除: paddle, matplotlib, pandas, llvmlite 等"
echo "  - 预期大小: 约 600-800MB" 