...
# 停止守护进程
if pgrep -f "daemon_monitor.py" > /dev/null; then
    echo "🛑 停止守护进程..."
...
# 检查是否还有进程在运行
if pgrep -f "main_monitor_gui_app.py" > /dev/null || pgrep -f "daemon_monitor.py" > /dev/null; then
    echo "⚠️ 强制终止剩余进程..."
    pkill -9 -f "main_monitor_gui_app.py" 2>/dev/null
    pkill -9 -f "daemon_monitor.py" 2>/dev/null
fi
